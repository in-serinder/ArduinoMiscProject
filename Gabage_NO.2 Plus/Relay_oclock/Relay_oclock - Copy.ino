...
const char *API_KEY = "0e4beb61ae3aa262060538e57e5c84db";
const char *CITY_CODE = "320724";
...
IPAddress ip;
WiFiUDP ntpUDP;
NTPClient tc(ntpUDP, NTP_SERVERS[2], ZONE, 60000);
...
void RelaySW() {
  ...
  if (swFlag) {
    dcount += 1;
    if (swOBJ != 0) {
      Relay_NO_swich(1, swOBJ);
    }

    if (dcount >= 2) {
      Relay_NO_swich(0, swOBJ);
      swFlag = false;
      dcount = 0;
      swOBJ = 0;    
    }
...
void getAPIData(String url) {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (Rcode > 0) {
      ...
      Serial.println(http.errorToString(Rcode).c_str());

    if (Rcode == -1 && retry_count < 3) {
      retry_count++;
      http.end();  // 先释放当前连接
      delay(1000);  
      getAPIData(url);  // 递归重试
      ...
      // 达到最大重试次数或非-1错误
      Serial.println("Max retries reached or non-recoverable error");
      retry_count = 0;  // 重置计数
    }
    }
  }
...
void RelayStatusINT() {
  String status = "";
  for (int i = 0; i < RELAY_LEN; i++) {
    status += String(RELAY_STATUS[i] ? 1 : 0);
  }
  LED_Flash(30);
  // Serial.println(status);
  pc.publish(MQTT_TOPIC, status.c_str());
...
void Relay_NO_swich(int sw, uint8_t pos) {
  ...
  switch (pos) {
    case 0:
      for (uint8_t i = 0; i < 6; i++) RELAY_STATUS[i] = sw;
      break;  //全部修改
    case 1:
...
void NtpUpdate() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("WiFi disconnected, skip NTP update");
    return;
  }

  bool ntpSuccess = false;
  int retryCount = 0;
  while (retryCount < 3 && !ntpSuccess) {

    ntpSuccess = tc.update();
    if (!ntpSuccess) {
  ...
  if (!ntpSuccess) {
    Serial.println("NTP sync failed after 3 retries");
    return;  // 同步失败，不更新RTC
  }


  unsigned long epochTime = tc.getEpochTime();
  const unsigned long MIN_VALID_TIME = 1577836800;  // 2020-01-01 00:00:00 UTC
  if (epochTime < MIN_VALID_TIME) {
    Serial.printf("Invalid NTP time: %lu (too old)\n", epochTime);
    return;
  }

  setTime(epochTime);  
  RtcDateTime current = RtcDateTime(
    year(), month(), day(), 
    hour(), minute(), second()
  );
...
void setRelay(uint8_t pos, boolean status) {
  switch (pos) {
    case 1: digitalWrite(Relay1, !status); break;
    case 2: digitalWrite(Relay2, !status); break;
    case 3: digitalWrite(Relay3, !status); break;
    default: RELAY_STATUS[pos - 1, status]; break;  //外定义继电器
  }
}
  ...
  if (pageflag == 1 && WeatherFlag) {
    ...
    for (uint8_t i = 0; i < WEATHERTEXTLEN; i++) {
      ...
      if (weather_statusindexarr[i] < CHINESEARRLEN) {
        if (weather_statusindexarr[i] == weather_statusindexarr[i + 1]) i++;              //在这里跳过重复查找索引
  ...
  if (pageflag == 2 && WeatherFlag) {
    ...
    for (uint8_t i = 0; i < 4; i++) {
      ...
      if (weather_directionarr[i] < CHINESEARRLEN) {
        if (weather_directionarr[i] == weather_directionarr[i + 1]) i++;                //在这里跳过重复查找索引
